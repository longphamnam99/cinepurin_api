...
export class PaymentsService {
    constructor(
        ...
        private readonly iPaymentsRepository: IPaymentsRepository
    ) {}
    ...
    createUrl(req: Request): string {
        ...
        return config.vnp_Url;
    }

    async checkPayment(req: Request): Promise<boolean> {
        const config = {
            "vnp_TmnCode": "M6VTR8B5",
            "vnp_HashSecret": "FSVLTSBHYBOYTNVOVIWSLLSMNMTSEWFF",
            "vnp_Url": "https://sandbox.vnpayment.vn/paymentv2/vpcpay.html",
        ...
        try {
            ...
            if (response.data.vnp_ResponseCode == "00") {
                await this.iPaymentsRepository.create({
                    amount: response.data.vnp_Amount,
                    code: response.data.vnp_TxnRef,
                    bank: response.data.vnp_BankCode,
                    date: response.data.vnp_PayDate
                })
                return true;
            } else {
                // const search = await this.iPaymentsRepository.findOneBy(
                //     {
                //         code: "05220624"
                //     }
                // )
