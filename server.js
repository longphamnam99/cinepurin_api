...
app.get('/api/ThongKe/getMonth', function (req, res) {
    dbConn.query(
        'SELECT MONTH(ngayMuaVe) AS thang, YEAR(ngayMuaVe) AS nam, SUM(amount) AS doanhSo FROM cinepurin_web.thongke  GROUP BY YEAR(ngayMuaVe), MONTH(ngayMuaVe) ORDER BY nam, thang',
        [],
        function (error, results, fields) {
            if (error) throw error;
            return res.send(results);
...
app.get('/api/ThongKe/getPhim', function (req, res) {
    dbConn.query(
        'SELECT tenPhim, COUNT(*) AS soLuong from cinepurin_web.thongke GROUP BY tenPhim',
        [],
        function (error, results, fields) {
            if (error) throw error;
            return res.send(results);
...
app.delete('/api/QuanLyNguoiDung/XoaNguoiDung', function (req, res) {
    ...
    dbConn.query(
        'DELETE FROM cinepurin_web.datve WHERE taiKhoanNguoiDat = ? AND isConfirm = 0 ',
        [req.query.TaiKhoan],
        function (error, results, fields) {
...
app.get('/api/QuanLyRap/LayThongTinTheLoaiPhim', function (req, res) {
    dbConn.query(
        'SELECT * FROM cinepurin_web.theloaiphim',
        [],
        function (error, results, fields) {
            if (error) throw error;
            return res.send(results);
...
app.post('/api/QuanLyRap/AddTheLoaiPhim', function (req, res) {
    dbConn.query(
        'INSERT INTO cinepurin_web.theloaiphim (name) VALUES(?)',
        [req.body.tenTheLoai],
        function (error, results, fields) {
            if (error) throw error;
            return res.send(results);
...
app.put('/api/QuanLyRap/UpdateTheLoaiPhim', function (req, res) {
    dbConn.query(
        'UPDATE cinepurin_web.theloaiphim SET tenTheLoai=? WHERE id=?',
...
app.post('/api/QuanLyRap/DeleteTheLoaiPhim', function (req, res) {
    dbConn.query(
        'DELETE FROM cinepurin_web.theloaiphim WHERE id=?',
        [req.body.id],
        function (error, results, fields) {
            if (error) throw error;
            return res.send(results);
        ...
        dbConn.query(
            ...
            async (error, results0, fields) => {
                ...
                const final = {
                    ...
                    tenPhim: results0[0]?.tenPhim || null,
                    biDanh: results0[0]?.biDanh || null,
                    trailer: results0[0]?.trailer || null,
                    hinhAnh: results0[0]?.hinhAnh.toString() || null,
                    moTa: results0[0]?.moTa || null,
                    maNhom: 'GP09',
                    ngayKhoiChieu: results0[0]?.ngayKhoiChieu || null,
                    danhGia: results0[0]?.danhGia || null,
                    nhaSanXuat: results0[0]?.nhaSanXuat || null,
                    daoDien: results0[0]?.daoDien || null,
                    dienVien: results0[0]?.dienVien || null,
                    maTheLoaiPhim: results0[0]?.maTheLoaiPhim || null,
                    tenTheLoai: results0[0]?.tenTheLoai || null,
                    dinhDang: results0[0]?.dinhDang || null,
                };
                return res.send(final);
            },
        );
...
app.post('/api/QuanLyRap/AddCumRap', function (req, res) {
    dbConn.query(
        'INSERT INTO cinepurin_web.cumrap SET ? ',
        {
    ...
    dbConn.query(
        'SELECT * FROM cinepurin_web.cumrap where  maCumRap = ?',
        [req.body.maCumRap],
        async (error, results0, fields) => {
            if (error) throw error;
            for (const result0 of results0) {
                dbConn.query(
                    'INSERT INTO cinepurin_web.hethongrapvacumrap (hethongrap, cumrap) VALUES(?, ?)',
...
app.put('/api/QuanLyRap/SuaCumRap', function (req, res) {
    dbConn.query(
        'UPDATE cinepurin_web.cumrap SET maCumRap=?, tenCumRap=?, diaChi=? WHERE maCumRap = ?',
...
app.post('/api/QuanLyRap/XoaCumRap', function (req, res) {
    dbConn.query(
        'DELETE FROM cinepurin_web.cumrap WHERE maCumRap = ?',
        [req.body.maCumRap],
        function (error, results, fields) {
            if (error) throw error;
        },
    ...
    dbConn.query(
        ...
        async (error, results0, fields) => {
            ...
            for (const result0 of results0) {
                dbConn.query(
                    'DELETE FROM cinepurin_web.hethongrapvacumrap where  maCumRap = ? ',
                    [result0.cid],
                    async (error, results1, fields) => {
                        if (error) throw error;
                    },
...
app.put('/api/QuanLyRap/SuaRap', function (req, res) {
    dbConn.query(
        'UPDATE cinepurin_web.danhsachrap SET tenRap= ? WHERE maRap = ?',
...
app.post('/api/QuanLyRap/XoaRap', function (req, res) {
    dbConn.query(
        'DELETE FROM cinepurin_web.danhsachrap WHERE maRap = ?',
        [req.body.maRap],
        function (error, results, fields) {
            if (error) throw error;
        },
...
app.post('/api/QuanLyRap/ThemRap', function (req, res) {
    dbConn.query(
        ...
        async (error, results0, fields) => {
            ...
            for (const result0 of results0) {
                dbConn.query(
                    'INSERT INTO cinepurin_web.danhsachrap SET ? ',
                    {
...
app.get('/api/QuanLyPhim/LayDanhSachPhim', function (req, res) {
    var sql = 'SELECT * FROM phiminsert';
    let { dangchieu, dacbiet, sapchieu } = req.query;
    if (dangchieu != null) {
        sql = 'SELECT * FROM phiminsert WHERE ngayDuKien <= CURRENT_DATE';
    }
    if (sapchieu != null) {
        sql = 'SELECT * FROM phiminsert WHERE ngayDuKien >= CURRENT_DATE';
    }
    if (dacbiet != null) {
        sql = 'SELECT * FROM phiminsert WHERE dacBiet = 1';
    }
...
app.put('/api/QuanLyDatVe/ThayDoiTrangThaiDatVe', function (req, res) {
    ...
    dbConn.query(
        'update cinepurin_web.datve set isConfirm = 1 where maGhe = ? and taiKhoanNguoiDat = ?',
...
app.delete('/api/DeleteTicketOfUser', function (req, res) {
    ...
    dbConn.query(
        'DELETE FROM cinepurin_web.datve WHERE maGhe= ? AND taiKhoanNguoiDat = ?',
        [req.query.maGhe, req.query.taiKhoanNguoiDat],
        async (error, results, fields) => {
            if (error) throw error;
            return res.send('Success');
                ...
                dbConn.query(
                    ...
                    function (error, results2, fields) {
                        ...
                        for (const result2 of results2) {
                            ...
                            dbConn.query(
                                'INSERT INTO cinepurin_web.thongke SET ? ',
                                {
                                    tenPhim: result2.tenPhim,
                                    ngayMuaVe: new Date(),
...
app.put('/api/QuanLyDatVe/SuaLichChieu', async (req, res) => {
    dbConn.query(
        'UPDATE cinepurin_web.lichchieuinsert SET ngayChieuGioChieu= ?, giaVe= ? WHERE maLichChieu= ? ',
