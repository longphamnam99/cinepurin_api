...
app.get('/api/QuanLyRap/LayThongTinLichChieuPhim', function (req, res) {
    dbConn.query('SELECT * FROM phiminsert JOIN hethongrapvaphim ON phiminsert.maPhim = hethongrapvaphim.maPhim JOIN hethongrap ON hethongrap.hid = hethongrapvaphim.maHeThongRap JOIN theloaiphim ON phiminsert.maTheLoaiPhim = theloaiphim.id WHERE phiminsert.maPhim = ?', [req.query.MaPhim], async (error, results0, fields) => {
        if (error) throw error;
        let heThongRapChieu = [];
        for (const result0 of results0) {
